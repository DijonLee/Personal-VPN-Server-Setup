...
Do you with to login to teh ADMIN UI as "openvpn"?  Press Enter (use default)
...
```
* _Note you may not be able to access on chrome. If so try a different browser*

Go to ```droplet_ip_address:943/admin``` and login using ```username: openvpn``` and password ``` enter_some_password_you_setup```. 
Agree to the ToS. If they harass you do download the app. log out and log in or try to bpyass it.

Import profile
Connect using openVPn =  username: openvpn@your.droplet.ip.address password: some_password_you_setup

- [Related Videos](https://www.youtube.com/watch?v=Bc8Nriy8_cE)
